...
const WFAROverviewTable = (props) => {
	...
	}, [weekBrackets, semesterNoOfWeeks, facultiesWithWfars]);


	return (
		<div className={styles.tableContainer}>
			<table className={styles.table} id="tableWFAROverview">
				<tr>    {/* Row 1 */}
					<th>
						Faculty{" "}
				...
				{facultiesWithWfars !== null && facultiesWithWfars.map((faculty, index) => {
					return (
						<tr>
							<td><strong>{faculty.last_name}</strong>, {faculty.first_name}</td>
							{faculty.wfars.map((wfars, index) => {
								return <td>
								{wfars.status === 1 && <span className={styles['noSubmission']}>No submission</span>}
								{wfars.status === 2 && <span className={styles['forChecking']}>For Checking</span>}
								{wfars.status === 3 && <span className={styles['ok']}>
								...
								{wfars.status === 4 && <span className={styles['withRevisions']}>With Revisions</span>}
								</td>
							})}
							{tdWfars[index]}
						</tr>);
