import styles from "./Status_&Checked.module.css";
import React, { Fragment } from "react";
const Status_Check = (props) => {
  //const choice = ;
  console.log(props.status);
  return (
    <Fragment>
      {props.status && props.checked_at && (
        <div className={styles.mainContainer}>
          <div className={styles.status_CheckContainer}>
            <h3>Status:</h3>
            <h3>Checked:</h3>
          </div>
          <div className={styles.dataContainer}>
            ...
            {props.status == 4 && <h3>With revisions</h3>}
            {props.checked_at && <h3>{props.checked_at}</h3>}
          </div>
        </div>
      )}
    </Fragment>
